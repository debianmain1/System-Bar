...
use_theme = "black"	        -- can be 'red','green','silver','gold','black','maroon','sienna', 'purple' or 'default'
...
show_netinfo = "no"		-- yes, show local + public IP
...
updates=tonumber(conky_parse('${updates}'))
	...
	-- gpu temp
	if show_gpu_temp == "yes" then
		retx,rety=conky_wired_tab_gpu_temp(startx,rety+tabvsep,150,90,"GPU Temp","Technical CE",13,themes.tred,themes.tgreen,themes.tblue)
	end

	-- show processes
	if show_processes == "yes" then
		-- 5 is the number of processes shown
		retx,rety=conky_wired_tab_process(startx,rety+tabvsep,150,100,"Processes",5,"Technical CE",13,themes.tred,themes.tgreen,themes.tblue)
	end
	...
	if show_netinfo == "yes" then
		retx,rety=conky_wired_tab_netinfo(startx,rety+tabvsep,150,100,"Net","Technical CE",13,themes.tred,themes.tgreen,themes.tblue)
	end
...
-- parameters idem conky_wired_tab_gpu_temp
function conky_wired_tab_gpu_temp(x,y,width,height,text,font,font_size,r,g,b)
	local vuheight=15  --VU barheight
	local onebarheight=vuheight+8
	local basey=y
	local barfs=12     --VU bar fontsize
	...
	if height < 60 then
		print("ERROR conky_wired_tab_gpu: insufficient height. Must be higher than 60")
		return
	end
	...
	conky_wired_bar_temp(x+6,y+onebarheight,vuheight,"GPU1","Technical CE",barfs,tonumber(conky_parse("${execi 5 nvidia-settings -t -q [gpu:0]/GPUCoreTemp}")))
	conky_wired_bar_temp(x+6,y+54,vuheight,"GPU2","Technical CE",barfs,tonumber(conky_parse("${execi 5 nvidia-settings -t -q [gpu:1]/GPUCoreTemp}")))

	return (x+width),(y+height)

end -- conky_wired_tab_gpu
...
function conky_wired_tab_network(x,y,width,height,text,font,font_size,r,g,b)
	...
	conky_lctext(x+6,y+30,"Upspeed ","Technical CE",13,themes.tred,themes.tgreen,themes.tblue)
	conky_lctext(x+6,y+46,"Downspeed ","Technical CE",13,themes.tred,themes.tgreen,themes.tblue)
	conky_lctext(x+6,y+62,"Up Total ","Technical CE",13,themes.tred,themes.tgreen,themes.tblue)
	conky_lctext(x+6,y+78,"Down Total ","Technical CE",13,themes.tred,themes.tgreen,themes.tblue)
...
function conky_wired_tab_system(x,y,width,height,text,font,font_size,r,g,b)
	...
	conky_lctext(x+6,y+43,conky_parse("${execi 1000 cat /proc/cpuinfo|grep 'model name'|sed -e 's/model name.*: //'| uniq | cut -c 10-28}"),"Technical CE",12,themes.tred,themes.tgreen,themes.tblue)
	conky_lctexta(x+134,y+30,conky_parse("${execi 1000 /bin/cat /etc/*release* | grep PRETTY_NAME | cut -c 14-30}"),"Technical CE",12,themes.tred,themes.tgreen,themes.tblue)
	conky_lctext(x+6,y+57,"CPU Speed:","Technical CE",12,themes.tred,themes.tgreen,themes.tblue)
	conky_lctexta(x+104,y+57,conky_parse("${freq_g 1}"),"Technical CE",12,themes.tred,themes.tgreen,themes.tblue)
	conky_lctext(x+98,y+57,"GHZ","Technical CE",12,themes.tred,themes.tgreen,themes.tblue)
	conky_lctext(x+6,y+70,"Kernel:","Technical CE",12,themes.tred,themes.tgreen,themes.tblue)
	conky_lctexta(x+150,y+70,conky_parse("$kernel"),"Technical CE",12,themes.tred,themes.tgreen,themes.tblue) 
...
function conky_wired_tab_netinfo(x,y,width,height,text,font,font_size,r,g,b)
	...
	cairo_stroke (cr)
		...
		conky_lctext(x+6,y+30,"Local IP ","Technical CE",10,themes.tred,themes.tgreen,themes.tblue)
		conky_lctext(x+6,y+42,"Public IP ","Technical CE",10,themes.tred,themes.tgreen,themes.tblue)		
		conky_lctexta(x+150,y+30,conky_parse("${addr eth0}"),font,font_size,r,g,b)	
		conky_lctexta(x+150,y+42,conky_parse("${execi 1000 wget -O - -q icanhazip.com}"),font,font_size,r,g,b)
